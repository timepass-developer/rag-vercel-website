...
const DocumentProcessingNode: React.FC<DocumentProcessingNodeProps> = ({ data }) => {
  ...
  return (
    <div className="relative bg-white dark:bg-gray-800 border rounded-md p-3 shadow-sm min-w-[220px]">
      ...
      <div className="mb-2">
        <label className="block text-xs font-medium mb-1">Upload Document</label>
        <input
          type="file"
      ...
      <div className="mt-2">
        <button
          ...
          className={`px-3 py-1 rounded-md text-white text-xs ${
            hasFiles ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'
          }`}
        >
          {data.isExecuting ? 'Processing...' : 'Process Document'}
        </button>
      </div>
